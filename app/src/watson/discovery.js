...
export function getBrandAlerts(brandName, lastUpdatedAt = null) {
  const params = {
    query: `${encodeURIComponent(brandName)},enriched_title.entities.type:Company,enriched_title.entities.relevance>0.8,source_type:mainstream, enriched_text.concepts.text:"women"`,
...
export function getProductAlerts(productName, lastUpdatedAt = null) {
  const params = {
    query: `${encodeURIComponent(productName)},source_type:mainstream, enriched_text.concepts.text:"women"`,
    filter: 'language:en,source_type:mainstream',
    aggregation: 'timeslice(crawl_date, 1day, America/Los_Angeles)',
    return: 'text,title,url'
  }
...
export async function getRelatedBrands(brandName, lastUpdatedAt = null) {
  ...
  const taxonomies = brandResults.results
    ...
    .map((taxonomyCount) => `(enriched_text.categories.label:/${taxonomyCount.label},enriched_text.categories.score>0.5, enriched_text.concepts.text:"women")`)
